...
def test_equality_with_booleans_and_ints_is_value_based(other: Any, expected: bool) -> None:
    """Boolean compares by value with plain bools and ints."""
...
@pytest.mark.parametrize("other", ["a string", 1.0, None])
def test_equality_with_non_integers_raises(other: Any) -> None:
    """Boolean compared to a non-integer value raises TypeError."""
    name = type(other).__name__
    with pytest.raises(TypeError) as exception_info:
